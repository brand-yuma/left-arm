...
2. `clients/[クライアント名]/` が存在しない場合は `/create-client [クライアント名]` を先に実行するよう案内する。
3. `clients/[クライアント名]/generate/01_brand/` に `brand-guideline.md` を以下の厳格なフォーマットで保存する。
4. 保存完了後、パス `clients/[クライアント名]/generate/01_brand/brand-guideline.md` を報告し「次に `/generate-banner [クライアント名] [要件]` と入力してください」と案内する。
